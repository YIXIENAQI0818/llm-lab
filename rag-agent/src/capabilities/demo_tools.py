...
def _index_documents_stub(path: str) -> str:
    return "文档已索引"
...
def create_demo_tools(plan_mgr=None, ltm=None, es=None) -> list[dict]:
    """创建绑定好的示例工具列表。

    Args:
        plan_mgr: PlanManager 实例，None 时不启用计划工具
        ltm: LongTermMemory 实例，None 时不启用 save_memory
        es: EmbeddingStore 实例，None 时不启用 RAG 工具（index_documents, search_docs）
    """
    ...
    tools = [
        ...
        },
        {
            ...
            "description": (
                "索引指定目录下的所有 Markdown 文档到知识库。"
                "传入一个目录路径，会自动读取所有 .md 文件，"
                "按 token 数分块后存入向量数据库，供 search_docs 检索。"
                "首次使用时调用一次即可。"
            ),
            "parameters": {
                "type": "object",
                "properties": {"path": {"type": "string", "description": "Markdown 文件所在目录路径"}},
                "required": ["path"],
            },
            "fn": _index_documents_stub,
        },
        {
            "name": "search_docs",
            "description": (
    ...
    for t in tools:
        ...
        elif t["name"] == "index_documents" and es is not None:
            def _index_docs(path, _es=es):
                try:
                    ...
                    total_chunks = 0
                    for doc in docs:
                        chunks = chunker.chunk(doc["content"], doc["name"])
                        for c in chunks:
                            _es.add("documents", c["text"], c["meta"])
                        total_chunks += len(chunks)
                    summary = [f"索引完成：{len(docs)} 个文件 → {total_chunks} 个片段"]
                    for doc in docs:
                        n = len(chunker.chunk(doc["content"], doc["name"]))
                        summary.append(f"  {doc['name']}: {n} chunks")
                    return "\n".join(summary)
                except Exception as e:
                    return f"索引失败: {e}"
            t["fn"] = _index_docs

        elif t["name"] == "search_docs" and es is not None:
            def _search_docs(query, top_k=3, _es=es):
                results = _es.search("documents", query, top_k=top_k, threshold=0.3)
                if not results:
                    return "未找到相关文档"
                lines = []
                for r in results:
